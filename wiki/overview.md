...
- Catalog entries can now omit a default system, which makes system choice a run-time clarification step instead of a required saved field.
...
- When an OpenAI-compatible backend is configured, runtime-argument extraction is now LLM-assisted: the model proposes candidate label/value pairs from freer-form phrasing, and the resolver still validates those candidates against probed CLI help before execution.
...
- Saved systems can now set `python_command`, so `python_script` entries keep the script path in `target` while using a system-specific interpreter like `python3.8`.
