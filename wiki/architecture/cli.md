...
- when a saved test stops because required runtime arguments are still missing, the session can hold that pending saved test context and accept a follow-up like `for 10 iterations`, `iteration is 10`, or `name is John`, rewriting it into a retry of the same saved test on the same chosen system
